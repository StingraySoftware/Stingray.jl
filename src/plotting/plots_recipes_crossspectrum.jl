...
"""
@recipe function f(cs::CrossSpectrum{T}; 
                  plot_type=:amplitude,
                  xscale=:log10,
                  yscale=:log10,
                  show_errors=false,  
                  ...
                  fontsize=8,
                  color=:blue,
                  show_noise_level=false) where T
    ...
    elseif plot_type == :coherence
        ...
        coherence_plot = abs2.(cs.power[freq_mask]) ./ (cs.ps1[freq_mask] .* cs.ps2[freq_mask])
    ...
    elseif plot_type == :pds_comparison
        ...
        error("Unknown plot_type: $plot_type. Available: :amplitude, :power, :phase_lag, :time_lag, :coherence, :snr, :cross_power_raw, :real_imaginary, :pds_comparison")
    end
end
...
"""
@recipe function f(cs::CrossSpectrum{T}, ::Val{:significant_detections}; threshold=3.0) where T
    
    if is_averaged(cs)
        title --> "Averaged Cross Spectrum - Significant Detections >$(threshold)σ ($(cs.m) segments)"
    else
...
"""
@recipe function f(cs::CrossSpectrum{T}, ::Val{:phase_lag_errors}; 
                  freq_range=nothing) where T
...
"""
@recipe function f(cs::CrossSpectrum{T}, ::Val{:time_lag_errors}; 
                  freq_range=nothing) where T
...
"""
@recipe function f(cs::CrossSpectrum{T}, ::Val{:white_noise}; high_freq_fraction=0.2) where T
...
@recipe function f(cs::CrossSpectrum{T}, ::Val{:noise_diagnosis}) where T
    ...
    aliasing_detected = length(high_freq_mask) > 0 && 
    ...
    empirical_noise = length(high_freq_idx) > 0 ? median(abs.(cs.power[high_freq_idx])) : theoretical_noise
...
"""
@recipe function f(cs_array::Vector{CrossSpectrum{T}}, 
                  ::Val{:normalization_comparison}; 
                  norm_labels=["Leahy", "Fractional RMS", "Absolute RMS"]) where T
    ...
    for (i, cs) in enumerate(cs_array)
        @series begin
            ...
            xlabel --> "Frequency (Hz)"
            ylabel --> i == 1 ? "Leahy cross-power" : (i == 2 ? "RMS cross-power" : "Absolute cross-power")
            xaxis --> :identity
            yaxis --> :log10
            cs.freq, abs.(cs.power)
        end
...
"""
@recipe function f(cs::CrossSpectrum{T}, ::Val{:lag_frequency_errors}; 
                  rebin_factor=1,
                  freq_range=nothing,
                  reference_lag=nothing) where T
...
- Optional reference lines for expected values
"""
@recipe function f(cs::CrossSpectrum{T}, ::Val{:phase_frequency_errors}; 
                  freq_range=nothing,
                  reference_phase=nothing) where T
...
"""
@recipe function f(cs_original::CrossSpectrum{T}, 
                  cs_rebinned::CrossSpectrum{T}, 
                  ::Val{:rebinning_comparison}; 
                  rebin_type="Linear") where T
...
"""
@recipe function f(lc1::LightCurve, lc2::LightCurve; 
                  colors=[:blue, :red],
