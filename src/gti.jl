...
function create_gti_mask(
    ...
    return mask,
    mapreduce(permutedims, vcat, filtered_gtis, init = reshape(eltype(gtis)[], 0, 2))
end
...
"""
function create_filtered_lightcurve(
    lc::LightCurve{T},
...
"""
function fill_bad_time_intervals!(
    el::EventList,
    gtis::AbstractMatrix{<:Real};
    dt::Real = 1.0,
    random_fill_threshold::Real = 10.0,
    ...
    for i = 1:size(btis, 1)
        ...
        if n_synthetic > 0
            ...
            synthetic_times =
                sort!(rand(rng, n_synthetic) .* effective_duration .+ effective_start)
            ...
            if !isnothing(el.energies)
                ...
                for j = 1:size(gtis, 1)
                    ...
                    if abs(gti_stop - bti_start) < 1e-10 ||
                       abs(gti_start - bti_stop) < 1e-10
