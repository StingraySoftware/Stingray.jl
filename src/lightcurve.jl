...
"""
function apply_event_filters(times::TimeType, energies::Union{Nothing,TimeType}, 
                            tstart::Union{Nothing,Real}, tstop::Union{Nothing,Real},
                            energy_filter::Union{Nothing,Tuple{Real,Real}}) where {TimeType<:AbstractVector}
...
"""
function bin_events(times::TimeType, bin_edges::Vector{T}) where {TimeType<:AbstractVector, T}
...
"""
function calculate_additional_properties(times::TimeType, energies::Union{Nothing,EnergyType}, 
                                        bin_edges::Vector{T}, bin_centers::Vector{T}) where {TimeType<:AbstractVector, EnergyType<:AbstractVector, T}
    properties = Vector{EventProperty}()
...
```
"""
Base.getindex(lc::LightCurve, r::UnitRange{Int}) = [(lc.timebins[i], lc.counts[i]) for i in r]
...
```
"""
Base.iterate(lc::LightCurve) = isempty(lc.timebins) ? nothing : ((lc.timebins[1], lc.counts[1]), 2)
...
"""
Base.iterate(lc::LightCurve, state) = state > length(lc.timebins) ? nothing : ((lc.timebins[state], lc.counts[state]), state + 1)