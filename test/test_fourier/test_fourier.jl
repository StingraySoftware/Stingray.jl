function compare_tables(table1, table2; rtol = 0.001, discard = [])
    ...
    data_fields_table1 = filter(
        field -> !(typeof(field) == Symbol && startswith(string(field), "meta")),
        names(table1),
    )
    data_fields_table2 = filter(
        field -> !(typeof(field) == Symbol && startswith(string(field), "meta")),
        names(table2),
    )
...
function setup_fourier_test_data(;
    dt = FOURIER_DT,
    len = FOURIER_LEN,
    ctrate = FOURIER_CTRATE,
    two_datasets = false,
)
    N = len ÷ dt
    dt = len / N
...
function test_events_vs_counts_pds(
...
function test_events_vs_counts_cs(
    use_common_mean::Bool,
    norm::String,
    include_errors::Bool = false,
)
...
let
    ...
    out_ev =
        avg_pds_from_events(_times, gti, FOURIER_SEGMENT_SIZE, FOURIER_DT, silent = true)
...
let
    test_pds_common_mean("frac")
end
let
    test_pds_common_mean("abs")
end
let
    test_pds_common_mean("none")
end
let
    test_pds_common_mean("leahy")
end

let
    test_cs_common_mean("frac", true, true)
end
let
    test_cs_common_mean("frac", true, false)
end
let
    test_cs_common_mean("frac", false, true)
end
let
    test_cs_common_mean("frac", false, false)
end

let
    test_events_vs_counts_pds(true, "frac")
end
let
    test_events_vs_counts_pds(false, "frac")
end
let
    test_events_vs_counts_pds(true, "frac", true)
end

let
    test_events_vs_counts_cs(true, "frac")
end
let
    test_events_vs_counts_cs(true, "frac", true)
end
...
let
    ...
    result_eventlist =
        avg_pds_from_eventlist(eventlist, FOURIER_SEGMENT_SIZE, data.dt, silent = true)
    result_times = avg_pds_from_events(
        data.times,
        data.gti,
        FOURIER_SEGMENT_SIZE,
        data.dt,
...
let
    ...
    for norm in ["frac", "abs", "none", "leahy"]
        result_eventlist = avg_pds_from_eventlist(
            eventlist,
            FOURIER_SEGMENT_SIZE,
            data.dt,
            norm = norm,
...
let
    ...
    for norm in ["frac", "abs", "none", "leahy"]
        for use_common_mean in [true, false]
            result_lightcurve = avg_pds_from_lightcurve(
                lc,
                norm = norm,
                use_common_mean = use_common_mean,
                silent = true,
            )
            result_iterable = avg_pds_from_iterable(
                flux_iterable,
                dt,
                norm = norm,
                use_common_mean = use_common_mean,
...
let
    ...
    result_iterables =
        avg_cs_from_iterables(flux_iterable1, flux_iterable2, dt, silent = true)
