@testset "LightCurve Tests" begin
    ...
    @testset "Error Methods" begin
        ...
        # Test Poisson errors (default)
        lc_poisson = create_lightcurve(events, 1.0)
        @test lc_poisson.err_method == :poisson
        # For Poisson: error = sqrt(counts), with sqrt(1) for zero counts
        expected_poisson = [c == 0 ? sqrt(1) : sqrt(c) for c in lc_poisson.counts]
        @test all(lc_poisson.count_error .≈ expected_poisson)
        
        # Test explicit Poisson errors
        lc_poisson_explicit = create_lightcurve(events, 1.0, err_method=:poisson)
        @test lc_poisson_explicit.err_method == :poisson
        @test all(lc_poisson_explicit.count_error .≈ expected_poisson)
        
        # Test Gaussian errors with provided error values
        # First create a Poisson light curve to determine the actual number of bins
        lc_temp = create_lightcurve(events, 1.0)
        num_bins = length(lc_temp.counts)
        custom_errors = rand(Float64, num_bins) .* 0.1 .+ 0.05  # Random errors between 0.05-0.15
        
        lc_gaussian = create_lightcurve(events, 1.0, err_method=:gaussian, 
                                       gaussian_errors=custom_errors)
        @test lc_gaussian.err_method == :gaussian
        @test all(lc_gaussian.count_error .≈ custom_errors)
        
        # Test Gaussian errors without providing error values (should fail)
        @test_throws ArgumentError create_lightcurve(events, 1.0, err_method=:gaussian)
        
        # Test Gaussian errors with wrong length (should fail)
        wrong_length_errors = Float64[0.1, 0.2, 0.3]  # Definitely wrong: num_bins + 1
        if length(wrong_length_errors) == num_bins
            # If by chance it matches, make it definitely wrong
            wrong_length_errors = vcat(wrong_length_errors, [0.4])
        end
        @test_throws ArgumentError create_lightcurve(events, 1.0, err_method=:gaussian,
                                                   gaussian_errors=wrong_length_errors)
        
        # Test invalid error method
        @test_throws ArgumentError create_lightcurve(events, 1.0, err_method=:invalid)
        
        # Test that Poisson method ignores provided gaussian_errors
        lc_poisson_with_unused_errors = create_lightcurve(events, 1.0, err_method=:poisson,
                                                         gaussian_errors=custom_errors)
        @test lc_poisson_with_unused_errors.err_method == :poisson
        @test all(lc_poisson_with_unused_errors.count_error .≈ expected_poisson)
        # Should not use the custom_errors when method is :poisson
        @test !(all(lc_poisson_with_unused_errors.count_error .≈ custom_errors))
    end
