...
@testset "Power Color Tests" begin
    ...
    @test_throws ArgumentError power_color(
        test_case.freq,
        test_case.power;
        freq_edges = [1],
    )
    ...
    pc0, _, pc1, _ =
        power_color(test_case.freq, test_case.power; freqs_to_exclude = [(1, 1.1)])
    ...
    pc0, pc0_err, pc1, pc1_err =
        power_color(test_case.freq, test_case.power; power_err = test_case.power / 2)
    pc0e, pc0e_err, pc1e, pc1e_err =
        power_color(test_case.freq, test_case.power; power_err = test_case.power)
