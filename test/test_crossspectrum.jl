...
function create_test_crossspectrum(freqs::Vector{Float64}, powers::Vector{Complex{Float64}};
...
function create_test_cseventlist(times::Vector{Float64}, energies::Union{Vector{Float64}, Nothing}=nothing)
    mock_headers = FITSIO.FITSHeader(["MJDREF"], [50000.0], ["Modified Julian Date reference"])
    gti = reshape([minimum(times), maximum(times)], 1, 2)
    
    mock_metadata = FITSMetadata(
        "test.fits", 1, "EVENTS", Dict{String, Vector}(), mock_headers, gti,
...
function create_test_cslightcurve(times::Vector{Float64}, counts::Vector{Int}, dt::Float64=1.0)
...
let
    ...
    mock_cs_single = create_test_crossspectrum(
        [1.0, 2.0, 3.0], [1.0+0im, 2.0+1im, 3.0-1im], m=1
    )
    
    mock_cs_averaged = create_test_crossspectrum(
        [1.0, 2.0, 3.0], [1.0+0im, 2.0+1im, 3.0-1im], m=5
    )
...
let
    cs_single = create_test_crossspectrum(
        [1.0, 2.0], [1.0+0im, 2.0+1im], 
        nphots1=100.0, nphots2=200.0, m=1
    )
    
    cs_averaged = create_test_crossspectrum(
        [1.0, 2.0], [1.0+0im, 2.0+1im], 
        nphots1=500.0, nphots2=1000.0, m=5
    )
...
let
    ...
    @test_logs (:warn, r"Estimated noise level .* much higher than theoretical") match_mode=:any begin
        props = noise_properties(cs)
        
        required_keys = ["theoretical_noise", "noise_level_1", "noise_level_2", 
                        "mean_power", "std_power", "mean_snr", "significant_detections",
                        "total_frequencies", "high_freq_power", "noise_to_signal_ratio",
                        "mean_rate_1", "mean_rate_2", "is_averaged", "segments_averaged",
...
let
    ...
    powers = [complex(0.1, 0.05), complex(5.0, 2.0), complex(0.2, 0.1), 
              complex(8.0, 3.0), complex(0.15, 0.08)]
    
    cs = create_test_crossspectrum(freqs, powers)
    
    @test_logs (:warn, "Too few high frequency points for reliable noise estimation") match_mode=:any begin
...
let
    ...
    @test_logs (:warn, "Too few high frequency points for reliable noise estimation") match_mode=:any begin
        ...
        required_keys = ["mean_snr", "max_snr", "significant_fraction", 
                        "noise_level", "dynamic_range", "is_averaged"]
...
let
    freqs = collect(0.5:0.5:10.0)
    powers = [complex(1.0 + 0.1*i, 0.1*i) for i in 1:length(freqs)]
...
let
    ...
    for i in 1:length(times)
        for _ in 1:counts1[i]
            push!(event_times1, times[i] + rand() * dt - dt/2)
        end
        for _ in 1:counts2[i]
...
let
    ...
    lc1 = LightCurve(times, dt, counts1, nothing, nothing, EventProperty{Float64}[], metadata1, :poisson)
    lc2 = LightCurve(times, dt, counts2, nothing, nothing, EventProperty{Float64}[], metadata2, :poisson)
