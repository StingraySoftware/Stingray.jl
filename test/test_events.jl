@testset "EventList Tests" begin
    @testset "Basic functionality" begin
        ...
        FITS(sample_file, "w") do f
            ...
            other_data = Dict{String,Vector{Float64}}("RATE" => Float64[21:30...])
    ...
    @testset "Missing columns" begin
        ...
        FITS(time_only_file, "w") do f
            write(f, Float32[])
            data = Dict{String,Vector{Float64}}("TIME" => Float64[1.0, 2.0, 3.0])
            write(f, data)
        end
        ...
        FITS(energy_only_file, "w") do f
            write(f, Float32[])
            data = Dict{String,Vector{Float64}}("ENERGY" => Float64[10.0, 20.0, 30.0])
            write(f, data)
        end
    ...
    @testset "Multiple HDUs with TIME columns" begin
        ...
        FITS(sample_file, "w") do f
            ...
            data2 = Dict{String,Vector{Float64}}("TIME" => Float64[21:30...])
            write(f, data2)
        end
