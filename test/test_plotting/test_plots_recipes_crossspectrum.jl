...
function create_lightcurve_test(
...
let
    ...
    powers[high_freq_idx] .=
        complex.(randn(sum(high_freq_idx)) .+ 0.5, randn(sum(high_freq_idx)))
    cs = create_crossspectrum(freqs, powers)
...
let
    ...
    coarse_spectrum = [
        mean(
            base_spectrum[max(1, round(Int, (f - 0.05) / 0.01)):min(
                end,
                round(Int, (f + 0.05) / 0.01),
            )],
