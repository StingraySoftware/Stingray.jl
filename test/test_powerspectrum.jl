...
let
    ...
    integrated_frac =
        sum(ps_frac.power[1:end-1] .* ps_frac.df) + ps_frac.power[end] * ps_frac.df / 2
    ...
    integrated_abs =
        sum(ps_abs.power[1:end-1] .* ps_abs.df) + ps_abs.power[end] * ps_abs.df / 2
...
let
    ...
    @test_nowarn aps_clustered =
        AveragedPowerspectrum(clustered_events, segment_size, dt = dt)
end
