...
@testset "test_coherence" begin
    ...
    @testset "test_intrinsic_coherence" begin
        coh = estimate_intrinsic_coherence.(cross, pds1, pds2, p1noise, p2noise, N)
...
@testset "test_fourier" begin
    ...
    @testset "test_fts_from_segments_cts_and_events_are_equal" begin
        ...
        fts_evts =
            collect(get_flux_iterable_from_segments(times, gti, segment_size, n_bin = N))
        fts_cts = collect(
            get_flux_iterable_from_segments(bin_times, gti, segment_size, fluxes = counts),
        )
        @test fts_evts == fts_cts
    end

    @testset "test_error_on_averaged_cross_spectrum_low_nave" for common_ref in
                                                                  [true, false]
    ...
    @testset "test_avg_pds_use_common_mean_similar_stats" for norm in [
        "frac",
        "abs",
        "none",
        "leahy",
    ...
    @testset "test_avg_cs_use_common_mean_similar_stats" for norm in [
            "frac",
            "abs",
            "none",
            "leahy",
        ],
        return_auxil in [true, false],
        fullspec in [true, false]
    ...
    @testset "test_avg_pds_cts_and_events_are_equal" for use_common_mean in [true, false],
    ...
    @testset "test_avg_pds_cts_and_err_and_events_are_equal" for use_common_mean in
                                                                 [true, false],
        norm in ["frac", "abs", "none", "leahy"]
    ...
    @testset "test_avg_cs_cts_and_events_are_equal" for use_common_mean in [true, false],
        norm in ["frac", "abs", "none", "leahy"]
    ...
    @testset "test_avg_cs_cts_and_err_and_events_are_equal" for use_common_mean in
                                                                [true, false],
        norm in ["frac", "abs", "none", "leahy"]
...
@testset "test_norm" begin
    ...
    @testset "test_poisson_level_$(norm)" for norm in ["abs", "frac", "leahy", "none"]
        pdsnorm =
            normalize_periodograms(pds, dt, N; mean_flux = mean, n_ph = nph, norm = norm)
        @test Statistics.mean(pdsnorm) ≈
              poisson_level(norm; meanrate = meanrate, n_ph = nph) rtol = 0.01
    end
    ...
    @testset "test_poisson_level_absolute_$(norm)" for norm in
                                                       ["abs", "frac", "leahy", "none"]
        pdsnorm = normalize_periodograms(
            pds,
            dt,
    ...
    @testset "test_normalize_none" begin
        pdsnorm =
            normalize_periodograms(pds, dt, N; mean_flux = mean, n_ph = nph, norm = "none")
